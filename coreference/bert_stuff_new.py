...
#config['cdlm_path'] =  '../cdlm/models/cdlm2/'
...
def generate_cdlm_embeddings_from_model(parallel_model,bert_sentences, device, batch_size=150):
    ...
    #get the overflowing token indices and chunk the document so that the special tokens are within max token length(4096)
    overflow_idx=[]
...
def generate_cdlm_embeddings(mention_map, vec_map_path , key_name ='bert_doc', num_gpus=4, batch_size=150, cpu=False):
    ...
    config_file_path: str
        Path to the RoBertA config file
    ...
    #cross_encoder.model = AutoModel.from_pretrained(os.path.join(bert_dir)).to(device)
