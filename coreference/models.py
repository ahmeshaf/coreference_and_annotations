...
import numpy as np
import pyhocon
#config_file_path = '/s/chopin/d/proj/ramfis-aida/coreference_and_annotations/coreference/models_dir/config_pairwise_long_reg_span.json'
...
#config.cdlm_path = '/s/chopin/d/proj/ramfis-aida/coreference_and_annotations/coreference/models_dir/'
...
class FullCrossEncoder(nn.Module):
    ...
    def forward(self, input_ids, attention_mask=None, arg1=None, arg2=None):
        ...
        #cls_vector = output[:, 0, :] #changed by Abhijnan to catch the tuple output in the right format, i.e first element  
...
class FullCrossEncoderSingle(nn.Module):
    ...
    def __init__(self, config, is_training=True, long=False):
        ...
        self.segment_size = config.segment_window * 2
        self.tokenizer = LongformerTokenizer.from_pretrained(config.cdlm_path)
        #self.tokenizer = RobertaTokenizerFast.from_pretrained(config.cdlm_path)
        #self.tokenizer = RobertaTokenizer.from_pretrained(config.cdlm_path)
        ...
        if not is_training and config.pretrained_model:
            ...
            self.model = AutoModel.from_pretrained(config.cdlm_path)
            #self.model = RobertaTokenizerFast.from_pretrained(config.cdlm_path)
            self.model.resize_token_embeddings(len(self.tokenizer))
            #print(self.model.embeddings.word_embeddings.weight[-1, :])
